...
def export_comprehensive_report(data):
    ...
    pdf.cell(0, 5, "STRATEGIC PERFORMANCE REPORT", ln=True, align='C')
    ...
    pdf.cell(0, 5, f"Analysis Date: {data['date']} | Prepared by: S&M Team Revenue Architect", ln=True)
    ...
    pdf.cell(50, 10, "KPI Category", 1, 0, 'C', True)
    pdf.cell(45, 10, "Target", 1, 0, 'C', True)
    pdf.cell(45, 10, "Actual", 1, 0, 'C', True)
    pdf.cell(50, 10, "Achievement", 1, 1, 'C', True)
    
    # 데이터 행 추가
    pdf.set_font("helvetica", "", 11)
    # 매출
    pdf.cell(50, 10, "Total Revenue", 1)
    pdf.cell(45, 10, f"KRW {data['tgt_rev']:,.0f}", 1)
    pdf.cell(45, 10, f"KRW {data['act_rev']:,.0f}", 1)
    pdf.cell(50, 10, f"{data['rev_pct']:.1f}%", 1, 1, 'R')
    # 객실수
    pdf.cell(50, 10, "Room Nights", 1)
    pdf.cell(45, 10, f"{data['tgt_rn']:,.0f} RN", 1)
    pdf.cell(45, 10, f"{data['act_rn']:,.0f} RN", 1)
    pdf.cell(50, 10, f"{data['rn_pct']:.1f}%", 1, 1, 'R')
    # ADR
    pdf.cell(50, 10, "Avg Daily Rate", 1)
    pdf.cell(45, 10, f"KRW {data['tgt_adr']:,.0f}", 1)
    pdf.cell(45, 10, f"KRW {data['act_adr']:,.0f}", 1)
    ...
    pdf.cell(0, 10, "2. Strategic Yielding Analysis", ln=True)
    ...
    pdf.set_fill_color(248, 245, 240)
    insight_text = (
        f"By implementing the 'Architect Strategy' (ADR +{data['adj_adr']}%), "
        f"we could have generated an additional Net Profit of KRW {data['gain']:,}. "
        "This indicates that high-tier yielding during demand acceleration periods "
        "is significantly more efficient than volume-driven sales strategies."
    )
    pdf.multi_cell(0, 10, insight_text, border=1, fill=True)
    
    pdf.ln(10)
    pdf.set_font("helvetica", "I", 10)
    pdf.cell(0, 10, "* This report was automatically generated by Amber Oracle Strategic System.", ln=True, align='C')
