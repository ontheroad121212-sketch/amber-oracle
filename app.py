...
with tabs[0]:
    ...
    st.info("💡 **[아키텍트 팩트 강제 주입]** 4/1~4/13까지의 검증된 OTB 데이터를 하드코딩했습니다. 1/3/4번은 PMS 좌표(G,H,N,AD) 기반 실시간 계산, 2번은 팩트 테이블을 추종합니다.")
    ...
    # 현재 시점 확정 (오늘이 4월 13일이므로 13일로 고정하여 과거 데이터 보호)
    ...
    # 2번 그래프용 데이터 구성 (주말 빈칸 보정)
    booking_pace_m = []
    month_facts = FACT_DB.get(selected_month, {})
    last_f_val = 0
    for d in range(1, curr_d + 1):
        if d in month_facts:
            last_f_val = month_facts[d]
        booking_pace_m.append(last_f_val / 100000000)
    ...
    # ==========================================
    # 🧠 3. PMS 데이터 추출 (1, 3, 4번 그래프 전용) - 팀장님 좌표 적용
    # ==========================================
    stay_pace, booking_evolution, act_c = [], [], []
    cur_rev_pms = 0
    
    if not df_full_pms.empty:
        p_df = df_full_pms.copy()
        
        # G:6(입실), H:7(퇴실), N:13(객실료), AD:29(예약일), B:1(상태)
        # 컬럼 인덱스로 정밀 접근
        p_df['Status'] = p_df.iloc[:, 1]
        p_df = p_df[~p_df['Status'].astype(str).str.contains('RC|취소|Cancel|NoShow', case=False, na=False)]
        
        p_df['In'] = pd.to_datetime(p_df.iloc[:, 6], errors='coerce')
        p_df['Out'] = pd.to_datetime(p_df.iloc[:, 7], errors='coerce')
        p_df['Rev'] = pd.to_numeric(p_df.iloc[:, 13].astype(str).str.replace(',', ''), errors='coerce').fillna(0)
        p_df['Bk'] = pd.to_datetime(p_df.iloc[:, 29], errors='coerce')
        p_df['Bk'] = p_df['Bk'].fillna(p_df['In'] - pd.Timedelta(days=1))
        
        p_df = p_df.dropna(subset=['In', 'Out'])
        
        # 🚨 [핵심] 타겟월 투숙분 일할 계산 (3월 입실 장기투숙객 반영 로직)
        ...
        # 👉 3번 그래프: 예약일 기준 진화 (PMS)
        # 4월 투숙분 매출이 예약일자별로 어떻게 쌓여왔는지 추적
        for d in trace_dt:
            if d > today_date: break
            # 해당 날짜 시점까지 예약된 4월 매출 총합
            evol_sum = p_df[p_df['Bk'] <= d.replace(hour=23, minute=59)]['Rev'].sum() # 근사치 계산
            booking_evolution.append(evol_sum / 100000000)

        # 👉 4번 그래프: 리드타임 (PMS)
        for d in range(-90, 1):
            lead_days = (p_df['In'] - p_df['Bk']).dt.days
            d_sum = p_df[lead_days >= -d]['Rev'].sum()
            act_c.append(d_sum / 100000000)

    # 상단 메트릭 강제 락온 (팩트 테이블 우선)
    display_rev = last_f_val if last_f_val > 0 else cur_rev_pms

    # ==========================================
    # 🧭 4. UI 출력 및 상태 진단
    # ==========================================
    expected_pct = pacing_curve_ratio[cur_idx] if cur_idx < len(pacing_curve_ratio) else 1.0
    forecast_rev = display_rev / expected_pct if expected_pct > 0 else display_rev
    
    st.markdown(f"### 🧭 현재 궤도 상태: **<span style='color:#00D1FF'>✅ 데이터 팩트 싱크 완료</span>**", unsafe_allow_html=True)
    m1, m2, m3, m4 = st.columns(4)
    m1.metric("순수 객실 매출 (OTB Fact)", f"{int(display_rev):,} 원")
    m2.metric("세이프존 기준점", f"{int(o_p[cur_idx] * 100000000):,} 원", f"{int(display_rev - o_p[cur_idx]*100000000):+,} 원")
    m3.metric("최근 7일 일평균 픽업", f"{int(velocity):,} 원/일")
    m4.metric("월말 예상 마감", f"{int(forecast_rev):,} 원")
    st.info(f"💡 **알림:** 3월 입실 장기 투숙객의 {selected_month}월 투숙분 매출이 1번 그래프에 일할 계산되어 포함되었습니다.")
    ...
    with r2c1:
        ...
        if booking_evolution: 
            fig3.add_trace(go.Scatter(x=trace_dt[:len(booking_evolution)], y=booking_evolution, name="Build-up", line=dict(color="#FFD700", width=3)))
