...
for i in range(12):
    ...
    if m in FACT_DB and FACT_DB[m]:
        m_rev = max(m_rev, max(FACT_DB[m].values()))
    if m == selected_month and cur_rev_sob > 0: 
        m_rev = max(m_rev, cur_rev_sob)
...
with tabs[4]:
    ...
    last_actual_idx = len(booking_pace_m) - 1
    
    if 0 <= last_actual_idx < num_d - 1:
        forecast_line[last_actual_idx] = booking_pace_m[-1]
        remaining_days = num_d - 1 - last_actual_idx
        step = (forecast_final_unit - booking_pace_m[-1]) / remaining_days
        for i in range(last_actual_idx + 1, num_d):
            forecast_line[i] = booking_pace_m[-1] + step * (i - last_actual_idx)
    elif last_actual_idx >= num_d - 1:
        forecast_line[-1] = booking_pace_m[-1]

    fig_fcst = go.Figure()
    fig_fcst.add_trace(go.Scatter(x=dates, y=o_p, name="Target S-Curve", line=dict(color="rgba(0,209,255,0.4)", dash="dash")))
    if booking_pace_m: fig_fcst.add_trace(go.Scatter(x=dates[:len(booking_pace_m)], y=booking_pace_m, name="Actual (OTB)", line=dict(color="#FF4B4B", width=4)))
    if any(v is not None for v in forecast_line): fig_fcst.add_trace(go.Scatter(x=dates, y=forecast_line, name="Forecast", line=dict(color="#FFD700", width=3, dash="dot")))
    ...
    with c1: st.metric("현재 누적 매출", f"{current_cum_rev_unit:.2f} 억")
    with c2: st.metric("월말 예상 매출", f"{forecast_final_unit:.2f} 억", f"{forecast_final_unit - target_goal_unit:+.2f} 억")
    with c3:
...
# ==========================================
# 탭 6. Firebase 기반 시장 분석 (상관관계 및 산점도 풀 복원)
# ==========================================
with tabs[6]:
    ...
    else: st.info("상관관계 분석을 위해 PMS 데이터를 업로드해 주세요.")
