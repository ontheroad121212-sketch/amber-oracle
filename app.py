...
with tabs[0]:
    ...
    # 🌟 수현님의 오리지널 오라클/세이프존 함수 100% 원상복구
    o_p, u_b, l_b = get_smart_corridor(tgt_m['rev']/100000000, t_dt, demand_idx)
    
    # ==========================================
    # 🧠 더블 카운팅(합계 행) 완벽 제거 및 진짜 OTB 궤도 생성
    # ==========================================
    clean_actual_pace = []
    velocity = 0
    cur_rev = current_rev_total # 732,471,320 원 (수현님의 원본 데이터)
    ...
    if not df_full_pms.empty:
        ...
        # 🚨 에러 원인 차단: 엑셀 하단의 '합계' 행을 무시하기 위해 박수(RN)가 0보다 큰 '진짜 예약 건'만 필터링
        if c_rn and c_rev_col and c_bk:
            ...
            # 1일부터 오늘(데이터 시점)까지의 찐 OTB 누적 곡선 생성 (합계 제외됨)
            ...
            # 가속도(Velocity): 최근 7일치 증가량을 일평균으로 환산
    ...
    # 마감 예측치 (수현님의 오리지널 o_p 함수 기반으로 재계산)
    expected_pct = (o_p[current_day_idx] / o_p[-1]) if (len(o_p) > 0 and o_p[-1] > 0) else 1.0
    forecast_rev = cur_rev / expected_pct if expected_pct > 0 else cur_rev
    
    # 상태 진단
    cur_upper = u_b[current_day_idx] * 100000000 if len(u_b) > current_day_idx else 0
    cur_lower = l_b[current_day_idx] * 100000000 if len(l_b) > current_day_idx else 0
    ideal_rev = o_p[current_day_idx] * 100000000 if len(o_p) > current_day_idx else 0

    if cur_rev > cur_upper:
        current_status = "🚨 예약 과속 (상한선 돌파)"
        status_color = "#FF4B4B"
        action_msg = "단가를 즉시 상향하여 예약 속도를 늦추고 객실 가치를 방어(Yielding) 하십시오."
    elif cur_rev < cur_lower:
        current_status = "⚠️ 예약 미달 (하한선 이탈)"
        status_color = "#FFD700"
        action_msg = "전환율(Conversion)을 높이기 위한 타겟 프로모션 또는 채널 노출도 강화가 필요합니다."
    else:
    ...
    with c1:
        ...
        if len(clean_actual_pace) > 0: 
            # 🌟 평행선 에러 차단: 데이터가 있는 '오늘'까지만 딱 끊어서 그리기
        ...
        fig1.update_layout(template="plotly_dark", height=400, title="매출 누적 궤도 및 예측")
        st.plotly_chart(fig1, use_container_width=True)
        
    with c2:
        # 🌟 수현님의 두 번째 그래프 (리드타임 D-90일 곡선) 무조건 100% 원상복구 🌟
        ...
        fig2.update_layout(template="plotly_dark", height=400, title="리드타임별 예약 곡선", xaxis_title="Days Before Arrival (투숙 D-Day)")
