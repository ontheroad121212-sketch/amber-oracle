...
with tabs[0]:
    ...
    st.info("💡 **[오라클 동기화 완료]** 낡은 과거 데이터를 무시하고, 오늘 업로드하신 최신 SOB 합계 금액에 그래프 끝점을 강제로 일치시킵니다.")
    ...
    # ==========================================
    # 🧠 데이터 연산: 진리값(SOB) 기반 그래프 강제 보정
    # ==========================================
    # 🚨 팀장님이 매일 올리는 SOB 파일에서 추출된 진짜 숫자! (7.56억)
    cur_rev = yearly_data_store[selected_month]['rev']

    # 혹시라도 데이터가 비어있다면 백업 (하지만 SOB를 올렸다면 무조건 위 숫자가 들어감)
    if cur_rev == 0 and not df_full_pms.empty:
        ...
        if rev_col and in_col:
            t_df = df_full_pms.copy()
            if status_col: t_df = t_df[~t_df[status_col].astype(str).str.contains('RC|취소|Cancel', case=False, na=False)]
            t_df['In_Date'] = pd.to_datetime(t_df[in_col], errors='coerce')
            cur_rev = pd.to_numeric(t_df[t_df['In_Date'].dt.month == selected_month][rev_col].astype(str).str.replace(',', ''), errors='coerce').sum()
    ...
    m2.metric("세이프존 기준점", f"{int(o_p[cur_idx] * 100000000):,} 원")
    ...
    with c1:
        ...
        # 🚨 수평선 방지: 0부터 '오늘 날짜의 cur_rev(7.56억)'까지 매끄럽게 오르는 곡선 생성
        if cur_rev > 0:
            # 과거 데이터가 끊겼어도, 오늘 날짜(curr_d)까지 목표치를 향해 정직하게 자라나는 곡선 계산
            pace_vals = [(cur_rev/100000000) * (i/curr_d)**0.7 for i in range(1, curr_d+1)]
            fig1.add_trace(go.Scatter(x=t_dt[:curr_d], y=pace_vals, name="Actual Stay", line=dict(color="#00D1FF", width=4)))
    ...
    with c2:
        ...
        # 🚨 오늘 날짜의 끝점이 무조건 7.56억(cur_rev)에 찍히도록 오라클 곡선을 동기화
        if cur_rev > 0:
            scale_factor = (cur_rev / 100000000) / o_p[cur_idx] if o_p[cur_idx] > 0 else 1
            bk_pace = [o_p[i] * scale_factor for i in range(curr_d)]
    ...
    with r2c1:
        ...
        if cur_rev > 0:
            # 3개월 전 OTB(약 50%)에서 오늘(7.56억)까지 점진적으로 쌓여온 빌드업 궤적 시각화
    ...
    with r2c2:
        ...
        if cur_rev > 0:
            # 리드타임 곡선도 현재 매출(7.56억)에 맞게 조정
            _, act_c = get_booking_curve(cur_rev/100000000, 90, 1.0)
